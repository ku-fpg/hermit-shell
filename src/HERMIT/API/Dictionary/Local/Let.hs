...
{-| 
  As let-subst-safe, but does not try to convert a recursive let into a 
  non-recursive let first.
-}
...
{-| 
  ...
  (\\ v1 -> let v2 = e1 in e2)  ==>  
  let v2 = e1 in (\\ v1 -> e2), if v1 is not free in e2.
...
{-| 
  ...
  ==> let v = ev in case s of { ... ; p -> e ; ... }
-}
letFloatCaseAltWith :: Int -> Rewrite LCore
letFloatCaseAltWith n = Transform $ method "letFloatCaseAlt" [toJSON (Just n)]
...
{-| 
  ...
  The argument list should contain the let-bound variables, in the desired 
  order.
-}
...
{-| 
  ...
  E.g. let {v1 = e1 ; v2 = e2 ; v3 = e3} in body ==> 
       case (e1,e2,e3) of {(v1,v2,v3) -> body}
-}
