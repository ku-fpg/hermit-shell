...
-- -- f (case s of alt -> e) ==> case s of alt -> f e
-- caseFloatArg :: String -> Rewrite LCore -> Rewrite LCore
...
-- | case-float = case-float-app <+ case-float-case <+ case-float-let <+ case-float-cast
...
-- | Float in a Case whose alternatives are parallel applications of the same function.
...
-- caseSplit :: String -> Rewrite LCore
...
-- caseSplitInline :: String -> Rewrite LCore
...
-- abstract out any occurrences of the alternative pattern using the case binder.
