...
{-| 
  innermost (unfold-basic-combinator <+ beta-reduce-plus <+ safe-let-subst <+ 
             case-reduce <+ let-elim)
-}
...
{-| 
  ...
  Note: be sure that the new rewrite either fails or makes progress, else this 
  may loop.
-}
